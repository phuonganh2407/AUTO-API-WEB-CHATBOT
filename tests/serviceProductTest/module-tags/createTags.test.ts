import { duplicateName, emptyName, fullTagsData, longName } from "../../../data/serviceProductData/tags.data";
import { createTag } from "../../../services/productApi/tags/tag.api";
...
describe("Thêm mới Thẻ tags cho Khách hàng, Nhà cung cấp, Sản phẩm, Đơn hàng", () => {
  ...
  test("@smoke TAG_ADD_001 - Tạo thành công thẻ tag với data hợp lệ", async () => {
  ...
  test("@smoke TAG_ADD_002 - Tạo thất bại thẻ tag với name đã tồn tại", async () => {
    ...
    test("TAG_ADD_003 - Tạo thất bại thẻ tag với name > 25 kí tự", async () => {
  ...
  test("TAG_ADD_004 - Tạo thất bại thẻ tag với name rỗng", async () => {
  ...
  test("TAG_ADD_005 - Tạo thành công thẻ tag cho từng loại typeTags cụ thể", async () => {
