import { errorMessages } from "../../../../data/api/errorMessages";
import { createTagForDelete, idTagInvalid } from "../../../../data/api/serviceProductData/tags.data";
import { deleteTag } from "../../../../services/api/productApi/tags/tag.api";
import { testsCheckFails } from "../../../../utils/funtionHelper";
import { callWithAllure } from "../../../../utils/httpWithAllure";
import { testWithAllure } from "../../testWithAllure";

describe("Xóa Thẻ tags cho Khách hàng, Nhà cung cấp, Sản phẩm, Đơn hàng", () => {
  testWithAllure("@smoke TAG_DEL_006 - Xóa thành công thẻ tag chưa có dữ liệu liên kết", async () => {
    //Tạo thẻ tag trước khi xóa đảm bảo không bị dính tag đã có dữ liệu liên kết
    const tagId = await createTagForDelete();
    // Gọi API delete (attach request/response vào Allure)
    const deleteResponse = await callWithAllure(() => deleteTag(tagId), { name: 'deleteTag' });
    // Kiểm tra response status 204 (No Content)
    expect(deleteResponse.status).toBe(204);
  });

  testWithAllure("TAG_DEL_007 - Xóa thất bại thẻ tag có ID không tồn tại trong hệ thống", async () => {
    // Sử dụng tag có sẵn, giả sử có dữ liệu liên kết
    const tagId = await idTagInvalid(); // Thay bằng ID thực tế của tag có liên kết
    await testsCheckFails(
      callWithAllure(() => deleteTag(tagId), { name: 'deleteTag' }),
      403,
      errorMessages.product.deleteTagNoteExisted
    );
  });
});
